...
and the line's own Brier score. Chosen k = smallest k within 2 SE of the best."""
import numpy as np
import pandas as pd
...
ALL = LINES + ["OUT"]
KGRID = [0, 5, 10, 25, 50, 100, 200, 400, 1000, np.inf]
MIN_RATE = 0.01
...
labels = ["raw" if k == 0 else ("inf" if np.isinf(k) else str(int(k))) for k in KGRID]
chosen = {}
for side, label, n_ref in (("B", "BATTER", 75), ("P", "PITCHER", 150)):
    ...
    for line in LINES:
        ...
        ok = [j for j in range(len(KGRID)) if pt[j] - pt[best] <= 2 * gap_se[j]]
        pick = min(ok, key=lambda j: KGRID[j])            # smallest k among the ties
        chosen[(side, line)] = KGRID[pick]
        inf_j = len(KGRID) - 1
        gains = {lab: round(1e6 * (pt[inf_j] - pt[j]), 1) for j, lab in enumerate(labels)}
        B = brier[side][line] / REPEATS
        pb, bb = boot_means(B)
        bbest = int(np.argmin(pb))
        k_pick = KGRID[pick]
        rows.append({"line": line, **gains, "best": labels[best], "chosen": labels[pick],
                     f"own share @{n_ref}": "100%" if k_pick == 0 else ("0%" if np.isinf(k_pick) else f"{n_ref / (n_ref + k_pick):.0%}"),
                     "SE of gap (best vs inf)": round(1e6 * gap_se[inf_j], 1), "Brier best": labels[bbest]})
...
q = val["PA"] >= 25
print("\n=== context-neutral runs per PA x1000, chosen k, full data ===")
