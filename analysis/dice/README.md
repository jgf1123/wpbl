...
| Script | Question | Result |
|---|---|---|
| `alpha_sweep.py` | Sweep the weight under both metrics, for the SHIPPED constants (read from `wpbl.dice`, never copied) and a high-k contrast set | on 39 games at the shipped k, runs bottom at 0.30 and log loss at 0.40, so the shipped 0.375 sits between them and inside both 1-SE bands. **The old row -- runs 0.15-0.20 -- was measured on a hardcoded copy that had drifted from dice.py; fixed 23 Sep** |
...
| `times_seen.py` | The i'th time a batter has faced this pitcher this game, against her first. The matchup is held EXACTLY fixed, so raw run values are differenced with nothing modelled away | +0.028 (SE 0.033) at the second meeting. Looks small and is not: only 59% of first meetings fall in the pitcher's first inning, so the clock is compressed and a +0.09 step predicts only +0.052 here |
| `pitch_tempo.py` | Does a tiring pitcher throw longer or shorter plate appearances, and how variable is an inning as a unit of work? | pitches per PA do not drift (-0.13, -0.11, +0.00, all within 1 SE). What varies is batters faced, so a plain pitch count already burns faster for a struggling pitcher -- no surcharge needed. An inning is 17.5 pitches, SD 7.8; batting around is 1.7% |
| `tired_card.py` | What a fading column costs and looks like, in cells (the column was called "tired" when this was written; the script name is kept so links do not break) | one cell is 1/94 = +0.013 runs/BF, not 1/33. G3's full shift is +0.032 and moves 3 of her 33 cells; reaching the +0.09 step needs 2.82x G3 and prints an absurd card |
| `burn_or_capacity.py` | Is a reliever's faster burn a separate parameter, or just a smaller capacity? | **no burn parameter**: within an outing rate and capacity are algebraically identical; across days a 3x rate leaves relievers arriving tired every time against 5 days of actual rest. Also shows the cross-day track rarely binds: 4 outings in 5 start from zero |
| `capacity.py` | Where a pitcher's capacity comes from: is it her ROLE, or her? | **hers.** 23 of 38 arms worked both ways (68% of outings) and four threw longer in relief than in any start, so a role median measures the manager's plan. Forced usage is not in blowouts (contested and decided medians barely differ) but in WHICH ARM: top-five ceilings 82, the rest 54. Also: 133.9 pitches a team-game over 6.82 innings of a seven-inning game, 19.5 an inning, and a starter's first inning is 19.5 -- the old 17.5 was the pitcher-inning average with partials in |
| `rules_check.py` | Does `dice_rules.md` say what the engine does? | **yes, 720 of 720**: every line x base state x out count x d12 face agrees. The rulebook is written independently for a player, so this is what stops the printed rules and the code drifting apart |
