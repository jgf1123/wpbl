"""Cohort size 150 / 300 / 600 PA (BF): batters with structure C, pitchers with the
four-step tree. Nested: k tuned inside each build half for each size; scored on the
held-out half. Reports held-out runs error relative to 300."""
import numpy as np
import pandas as pd

_here = __file__
__file__ = _here.replace("cohort_cv.py", "greedy_cv.py")
